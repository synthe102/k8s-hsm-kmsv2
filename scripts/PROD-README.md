...
- `kubectl` configured against the target cluster.
- SSH access (with sudo) to all control-plane nodes.
- Python 3 available on every control-plane node (for the apiserver patching step).
...
- A **user PIN** with permissions to create and use AES-256 keys.
- (Optional) A pre-provisioned AES-256 KEK with label `k8s-kms-kek`.
  If the KEK does not exist yet, set `AUTO_CREATE_KEY=true` the first time.

### Build host

- Docker installed.
- Proteccio client package installed locally so you can copy the files listed below.
...
cp /etc/proteccio/proteccio.crt         deploy/proteccio/
# If mTLS is used:
# cp /etc/proteccio/proteccio_client.crt deploy/proteccio/
# cp /etc/proteccio/proteccio_client.key deploy/proteccio/   # keep out of git!
```

Verify the HSM is reachable before proceeding:

```bash
nethsmstatus
```

Expected output ends with `Token state: 0X40 OPERATIONAL`.
...
```bash
export REGISTRY=registry.example.com/myorg       # your container registry
export TOKEN_LABEL=HSM1-V1                        # PKCS#11 token label
export USER_PIN=<your-pin>                        # PKCS#11 user PIN
export CONTROL_PLANE_NODES="192.0.2.10 192.0.2.11 192.0.2.12"
export CONTROL_PLANE_SSH_USER=ubuntu
export CONTROL_PLANE_SSH_KEY=~/.ssh/my-key
export AUTO_CREATE_KEY=true                       # only on first deploy; set false later
```
...
The script performs these steps for **each control-plane node**:

```mermaid
sequenceDiagram
    participant S  as deploy-proteccio.sh
    participant D  as Docker daemon
    participant N  as Control-plane node
    participant KB as kubelet
    participant KP as KMS plugin pod
    participant AS as kube-apiserver
    participant HSM as Proteccio HSM

    S->>D: docker build (with libnethsm.so)
    S->>D: docker push → registry
    S->>N: scp kms-plugin-pod manifest (PIN + token injected)
    N->>KB: /etc/kubernetes/manifests/ updated
    KB->>KP: Start kms-plugin container
    KP->>HSM: PKCS#11 C_Initialize / C_Login
    KP-->>N: Unix socket ready
    S->>N: poll socket
    S->>N: scp encryption-config.yaml
    S->>N: run patch-apiserver.py
    KB->>AS: Restart with --encryption-provider-config
    AS->>KP: Status() gRPC → OK
    S->>AS: kubectl create secret (round-trip test)
```
...
|----------|----------|-------------|
| `REGISTRY` | No | Docker registry prefix (e.g. `registry.example.com/myorg`) |
| `IMAGE_TAG` | No | Image tag; defaults to `latest` |
| `TOKEN_LABEL` | **Yes** | PKCS#11 token label on the Proteccio HSM |
| `USER_PIN` | **Yes** | PKCS#11 user PIN |
| `KMS_KEY_LABEL` | No | Key label in the token; defaults to `k8s-kms-kek` |
| `AUTO_CREATE_KEY` | No | Set `true` to auto-provision the KEK (first deploy only) |
| `CONTROL_PLANE_NODES` | **Yes** | Space-separated list of control-plane IPs/hostnames |
| `CONTROL_PLANE_SSH_USER` | No | SSH user; defaults to `ubuntu` |
| `CONTROL_PLANE_SSH_KEY` | No | SSH key path; defaults to `~/.ssh/id_rsa` |
...
The `USER_PIN` is injected into the static pod manifest via `sed` at deploy time and
lives in `/etc/kubernetes/manifests/kms-hsm-plugin.yaml` on the node.  This file is
readable only by root.  If your threat model requires stronger PIN protection consider:

- Using an HSM-attached TPM to seal the PIN.
- Using a secrets management system (Vault, AWS Secrets Manager) to inject the PIN at
  node boot time via an init script before the static pod is started.
...
1. Provision a new AES-256 key on the Proteccio HSM (new label, e.g. `k8s-kms-kek-v2`).
2. Update the plugin deployment (`--key-label=k8s-kms-kek-v2`) on every node.
   The plugin returns the new `keyID` in subsequent `EncryptResponse` messages.
3. Force re-encryption of all existing secrets:
...
```bash
# Etcd snapshot example (adjust etcd endpoint and cert paths)
ETCDCTL_API=3 etcdctl snapshot save /backup/etcd-$(date +%Y%m%d).db \
  --endpoints=https://127.0.0.1:2379 \
  --cacert=/etc/kubernetes/pki/etcd/ca.crt \
  --cert=/etc/kubernetes/pki/etcd/server.crt \
...
```bash
# Check plugin logs on the node
ssh ubuntu@<node> "sudo crictl logs \$(sudo crictl ps --name kms-plugin -q)"
```

```
tail -f /var/log/proteccio.log    # Proteccio client log on the node
nethsmstatus                      # Verify HSM reachability
```
...
### Disable encryption (emergency rollback)

1. Remove `--encryption-provider-config` from the apiserver manifest on each node.
2. Force-re-encrypt (or delete and recreate) all secrets so they are stored as plaintext.
3. Remove the KMS plugin pod from `/etc/kubernetes/manifests/`.

> **Warning:** After rollback, all secrets that were never re-read will remain
> encrypted in etcd and will fail to decode until the KMS plugin is restored.
> Perform the rollback procedure only in a maintenance window.
